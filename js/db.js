...
async function placeOrder({ name, phone, address, slot, payment, items, total }) {
  ...
  const { data: products } = await db.from('products').select('id, farmer_id').in('id', productIds);
  if (products && products.length > 0) {
    const counts = {};
    products.forEach(p => counts[p.farmer_id] = (counts[p.farmer_id] || 0) + 1);
    const notifications = Object.keys(counts).map(fid => ({
      farmer_id: fid,
      order_id: orderId,
      message: `You have received a new order (#${orderId}) containing ${counts[fid]} item(s).`,
      is_read: false
    }));
...
// Fetch orders received by a specific farmer
async function getFarmerOrders(farmerId) {
  // Query order_items joined with products (to filter by farmer_id) and orders (to get buyer info)
  const { data, error } = await db
    .from('order_items')
    .select(`
      id, qty, price,
      products!inner ( id, name, image, farmer_id ),
      orders!inner ( id, name, phone, address, placed_at, current_step, payment )
    `)
    .eq('products.farmer_id', farmerId);
    
  if (error) { console.error('getFarmerOrders:', error); return []; }
  
  // Sort in JS since ordering a joined table can be tricky in some PostgREST versions
  return data.sort((a,b) => new Date(b.orders.placed_at) - new Date(a.orders.placed_at));
}

// ──────────────────────────────────────────────────────────────
//  NOTIFICATIONS
// ──────────────────────────────────────────────────────────────
async function getNotifications(farmerId) {
  const { data, error } = await db
    .from('notifications')
    .select('*')
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (user) {
    ...
    authLinks.forEach(link => {
      // Set to Dashboard if Farmer, else Profile
      if (user.user_metadata?.role === 'farmer') {
        ...
        link.innerHTML = `<i class="fa fa-user-circle"></i> ${firstName} <span style="font-size:0.75rem;margin-left:4px;opacity:0.8;">(Profile)</span>`;
        link.href = '#';
        link.onclick = (e) => e.preventDefault(); // Currently just a placeholder for consumers
      }
      
      // Inject dedicated Logout button next to it
      if (!link.nextElementSibling || !link.nextElementSibling.classList.contains('global-logout-btn')) {
        const logoutBtn = document.createElement('a');
        logoutBtn.href = '#';
        logoutBtn.className = 'btn btn-ghost btn-sm global-logout-btn';
